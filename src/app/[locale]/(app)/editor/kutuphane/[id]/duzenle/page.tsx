...
export default function EditorKitapDuzenlePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (!user) return;
    api.get(`/api/kutuphane/kitaplar/${id}`).then(r => {
      const k = r.data;
      setForm({
        baslik:          k.baslik ?? '',
        yazar:           k.yazar ?? '',
  ...
  const guncelle = useMutation({
    mutationFn: () => api.put(`/api/kutuphane/kitaplar/${id}`, {
      id,
      baslik:          form.baslik,
    ...
    <div className="min-h-[100dvh] bg-[#F3F4F6]">
      <main className="max-w-[680px] mx-auto px-4 py-10">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Kitabı Düzenle</h1>
            <p className="text-slate-500 text-sm mt-0.5 font-mono">{id}</p>
          </div>
        </div>

        {hata && !yukluyor && (
          <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-xl p-3 mb-4">{hata}</p>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5">
          ...
          <button
            ...
            disabled={!form.baslik || !form.url || guncelle.isPending}
